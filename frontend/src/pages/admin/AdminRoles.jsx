...
export default function AdminRoles() {
  ...
  // roleId -> candidate count, computed from a single participants fetch
  // instead of calling the API once per row.
  ...
  async function refresh() {
    setLoading(true);
    try {
      const [allRoles, allParticipants] = await Promise.all([db.getRoles(), db.getParticipants()]);
      setRoles(allRoles);
  ...
  async function handleSave() {
    ...
    try {
      if (editingRole) {
        await db.updateRole(editingRole.id, name);
  ...
  async function handleDelete() {
    try {
      await db.deleteRole(deleteTarget.id);
            ...
            {!loading &&
              roles.map((role, idx) => (
                <tr key={role.id}>
                  <td>{idx + 1}</td>
                  <td>{role.name}</td>
                  <td>{candidateCounts[role.id] || 0}</td>
